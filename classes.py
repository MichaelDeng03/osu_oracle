...
class Beatmap:
    ...
    def __init__(self, beatmap: ossapi.models.Beatmap):
        self.id = getattr(beatmap, "id", None)
    ...
    def insert(self, cursor):
        ...
        sql = """
        INSERT INTO beatmap (
            id, beatmapset_id, difficulty_rating, bpm, count_circles, count_sliders, 
...
class User:
    ...
    def __init__(self, user: ossapi.models.User):
        ...
        self.total_pp = getattr(statistics, "pp", None) if statistics else None
        self.hit_acc = getattr(statistics, "hit_accuracy", None) if statistics else None
        self.ranked_score = (
            getattr(statistics, "ranked_score", None) if statistics else None
        )
        self.play_count = (
            getattr(statistics, "play_count", None) if statistics else None
        )
        self.playtime = getattr(statistics, "play_time", None) if statistics else None
        ...
        self.total_hits = (
            getattr(statistics, "total_hits", None) if statistics else None
        )
        ...
        self.join_date = datetime.strftime(self.join_date, "%Y-%m-%d %H:%M:%S")
        self.update_date = strftime("%Y-%m-%d %H:%M:%S", localtime(time.time()))

    def insert(self, cursor):
        """Inserts into osu_recommender.db table"""
        sql = """
        INSERT INTO user (
            id, username, total_pp, hit_acc, ranked_score, play_count, playtime, count_100, count_50, 
...
class Score:
    ...
    def __init__(self, score: ossapi.models.Score):
        ...
        self.beatmapset_id = (
            getattr(beatmap, "beatmapset_id", None) if beatmap else None
        )
    ...
    def insert(self, cursor):
        ...
        sql = """
        INSERT INTO score (
            id, user_id, beatmap_id, beatmapset_id, mods, score, max_combo, perfect, count_50, count_100, 
...
class Beatmapset:
    ...
    def __init__(self, beatmapset: ossapi.models.Beatmapset):
        self.id = getattr(beatmapset, "id", None)
    ...
    def insert(self, cursor):
        ...
        INSERT INTO beatmapset (
            id, language, nsfw, play_count, ranked_date, tags, title, artist, author_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        cursor.execute(
            query,
